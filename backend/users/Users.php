	...
	class Users
	{
		...
		public function getUsers($params){
			...
			$sql = "SELECT 
						...
						contrasena,
						token
					FROM users $where ORDER BY id DESC LIMIT 0,10 ";
		...
		public function editUser($params){
            ...
            $sql="UPDATE users SET
						...
						username       = '$params[correo]',
						contrasena     = '$params[contrasena]'
					WHERE id=$params[id_user] ";
            $query=mysqli_query($this->mysql, $sql);
            if ($query) {
		...
		public function deleteUser($params){
        	...
        	$query=mysqli_query($this->mysql, $sql);
        	if ($query) {
				$response = array('response' => 'success', 'msg'=>'Se elimino correctamente' );
            }
            else {
				$response = array('response' => 'failed', 'msg'=>'No se elimino el usuario' );
            }
			return array('status' => 200, $response	);
		}
